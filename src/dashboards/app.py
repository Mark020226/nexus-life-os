...
with st.sidebar:
    ...
    st.info("Re-aprendizaje activo de materias clave + Fundamentos de IA, Software y Automatización.")
    
    st.markdown("---")
    st.markdown("### ⚡ Acceso Rápido")
    st.caption("Sistema local en tiempo real respaldado en Git & SQLite.")
...
cur.execute("SELECT COUNT(*) FROM academic_courses WHERE status='Cursando'")
courses_enrolled = cur.fetchone()[0]
...
with col1:
    st.metric("💡 Recursos en Segundo Cerebro", f"{total_resources}", help="Guardados y enlaces analizados de Instagram")
with col2:
    st.metric("🎓 Materias Aprobadas UMSA", f"{courses_approved} / {total_courses}", f"{int((courses_approved/total_courses)*100)}% avance")
with col3:
    st.metric("📚 Materias en Curso", f"{courses_enrolled}", "3er Semestre")
with col4:
    st.metric("🤖 Agentes Activos", "5 Agentes", "Orquestador, Académico, Finanzas, Salud, Segundo Cerebro")

st.markdown("---")

# Main Tabs
tab1, tab2, tab3, tab4 = st.tabs([
    "💡 Segundo Cerebro (Biblioteca)",
...
# ------------------------------------------------------------
# TAB 1: SEGUNDO CEREBRO
# ------------------------------------------------------------
with tab1:
    st.subheader("💡 Tu Biblioteca de Conocimiento y Enlaces Guardados")
    st.markdown("Aquí se concentran los **269 recursos, videos y enlaces** que tenías guardados en Instagram, organizados para que nunca más queden en el olvido.")
    ...
    with col_search:
        search_query = st.text_input("🔍 Buscar por palabra clave (ej. Claude, NASA, NVIDIA, Python, Prompt, Beca):", "")
    with col_cat:
        cat_filter = st.selectbox("Categoría:", ["Todas"] + categories)
    with col_status:
        status_filter = st.selectbox("Estado:", ["Todos", "Pendiente", "En Estudio", "Completado"])
        
    # Query resources
    query = "SELECT id, title, category, source, date_added, url, summary, status FROM resources WHERE 1=1"
    params = []
    ...
    for idx, row in df_res.iterrows():
        with st.expander(f"{row['category']} | {row['title']}"):
            st.markdown(f"**Origen:** {row['source']} | **Fecha:** {row['date_added']} | **Estado:** `{row['status']}`")
            if row['summary']:
                st.markdown(f"**Detalle / Transcripción:**\n\n{row['summary']}")
            if row['url']:
                st.markdown(f"👉 **Enlace directo:** [{row['url']}]({row['url']})")
                
    st.markdown("---")
    st.markdown("### ➕ Añadir Nuevo Enlace Manualmente")
    with st.form("add_resource_form"):
        f_url = st.text_input("URL del enlace / video:")
        ...
        f_summary = st.text_area("Notas / Puntos clave de valor:")
        submit_res = st.form_submit_button("Guardar en Segundo Cerebro")
        
        if submit_res and f_url:
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
            cur.execute("""
            INSERT INTO resources (url, title, summary, category, date_added, source, status)
            VALUES (?, ?, ?, ?, ?, 'Entrada Manual', 'Pendiente')
            """, (f_url, f_title or f_url, f_summary, f_cat, now_str))
            conn.commit()
